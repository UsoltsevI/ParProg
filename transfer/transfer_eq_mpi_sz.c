...
// 
// оптимизированный алгоритм: один процесс только под сборку данных
//
 ...
 */
double* send_uk_data(double* uk, double* uk1, int start, int end, int to_rank, int tag, int M, MPI_Request* request) {
 ...
 */
double* send_uk_to_root(double* uk, int k, int M, int root_rank, MPI_Request* request) {
    double* copy = (double *) malloc(M * sizeof(double));
    memcpy(copy, uk, M * sizeof(double));
    MPI_Isend(copy, M, MPI_DOUBLE, root_rank, calc_root_tag_uk(k), MPI_COMM_WORLD, request);
    return copy;
}
...
int main(int argc, char** argv) {
    ...
    MPI_Comm_size(MPI_COMM_WORLD, &size_cl);
    ...
    double** send_buffers = NULL;
    if (rank != size) {
        int max_requests = K / size + K + size;
        requests = (MPI_Request*) calloc(max_requests, sizeof(MPI_Request));
        if (requests == NULL) {
            fprintf(stderr, "Ошибка выделения памяти: %s\n", strerror(errno));
            MPI_Abort(MPI_COMM_WORLD, 1);
        }
        send_buffers = (double**) calloc(max_requests, sizeof(double*));
    ...
    if (rank == 0) {
        ...
        for (int part = 0; part < size; part++) {
            ...
            send_buffers[last_req] = send_uk_data(uk_next, phi, start, end, get_next_rank(rank, size), 
                calc_tag_uk(1, part, size), M, &requests[last_req]);
            last_req++;
        }

        send_buffers[last_req] = send_uk_to_root(uk_next, 1, M, size, &requests[last_req]);
        last_req++;
        free(phi);
    }
            ...
            for (int part = 0; part < size; part++) {
                ...
                if (start > 0) {
                    ...
                    if (k < K - 1) {
                        ...
                        // отправка (part - 1) !!!
                        send_buffers[last_req] = send_uk_data(uk_next, uk, prev_start, prev_end, 
                            get_next_rank(rank, size), calc_tag_uk(k + 1, part - 1, size), M, &requests[last_req]);
                        last_req++;
                    }
                }
                ...
                if (end == M && k < K - 1) {
                    assert(part == size - 1);
                    send_buffers[last_req] = send_uk_data(uk_next, uk, start, end, get_next_rank(rank, size), 
                        calc_tag_uk(k + 1, part, size), M, &requests[last_req]);
                    last_req++;
                }
            }

            send_buffers[last_req] = send_uk_to_root(uk_next, k + 1, M, size, &requests[last_req]);
            last_req++;
        }
    }
    ...
    if (rank == size) {
        ...
        MPI_Waitall(last_req - 1, requests, MPI_STATUSES_IGNORE);
